    ...
    <div className="container mx-auto"  style={{backgroundColor:"#f5efe7",minWidth:"100%"}}>
      <div className="flex-col p-2 lg:ml-10">
        ...
        >
          <button
            id="postJobBtn"
            className="mt-2 relative bg-blue-500 text-white font-semibold py-2 px-4 rounded-lg flex items-center space-x-2 hover:bg-blue-600 focus:outline-none focus:bg-blue-600 mb-4"
          >
        ...
        <div className="mx-auto mt-16 lg:grid lg:grid-cols-3 md:grid grid-cols-2">
          {loading ? (
            <p>Loading...</p>
          ) : (
        ...
        <div className="mb-3 flex items-center bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
          <div className="p-4 w-2/3">
            <h6 className="flex items-center font-bold text-lg mb-2 text-gray-900 dark:text-white">
              <FaBuilding size={16} className="text-blue-500 mr-2" /> {job.CompanyName}
            </h6>
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300 mb-1 flex items-center">
              <FaUserTie className="mr-2 text-green-500" /> Experience: {job.Experience}
            </p>
            <p className="text-sm text-gray-700 dark:text-gray-300 flex items-center">
              <FaTools className="mr-2 text-purple-500" /> Skills Required: {job.SkillsRequired}
            </p>
            {job.userDetails && (
              <p className="flex items-center text-sm text-gray-700 dark:text-gray-300 mb-1">
                <FaUser className="text-blue-500 mr-2" /> Posted By: {job.userDetails.name}
              </p>
            )}
            {job.matchPercentage > 0 &&
              <p className={`flex items-center text-sm font-bold text-gray-700 dark:text-gray-300 ${matchPercentageColor}`}>
                <FaChartBar className="text-green-500 mr-2" /> Skills Matching: {job.matchPercentage}%
              </p>
            }
          </div>
          <div className="flex flex-col justify-center items-center">
            <a href={job.ApplyLinks} rel="noreferrer" target="_blank" className="flex items-center justify-center text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm w-full sm:w-auto px-3 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
            >
