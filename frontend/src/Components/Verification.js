  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/api/verifyOTP', {
        ...
        body: JSON.stringify({ enrollmentNumber: enrollmentNumber, otp }),
      });
  ...
  return (
    <section className="bg-gray-50 dark:bg-gray-900">
      <div className="flex flex-col items-center mt-20 px-6 py-8 mx-auto md:h-screen lg:py-0">
        <div
          className=" rounded-md flex items-center mb-6 text-2xl font-semibold text-gray-900 dark:text-white"
        >
          OTP Verification
        </div>
