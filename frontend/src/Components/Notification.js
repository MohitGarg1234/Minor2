...
const Notification = ({ notifications, handleAccept, handleReject }) => {
      ...
      {notifications && notifications.map(notification => (
        <div key={notification._id}>
          ...
          {notification.status === 'pending' && (
            <div>
              <button onClick={() => handleAccept(notification._id, notification.message)}>Accept</button>
              <button onClick={() => handleReject(notification._id, notification.message)}>Reject</button>
            </div>
          )}
        </div>
      ))}
                ...
                <div className="flex items-center">
                  <div className="relative me-4">
                    ...
                    {notification.type === "connectionRequest" && (
                      <div>
                        <button onClick={() => handleAccept(notification._id)}>
                          Accept
                        </button>
                    ...
                    <span
                      className={`top-0 start-7 absolute w-3.5 h-3.5 border-2 border-white rounded-full ${
